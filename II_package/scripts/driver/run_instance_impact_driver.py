...
def parse_args():
    ...
    p.add_argument("--shared_script", type=Path, default=Path("scripts/shared/generate_shared_instance_subimages.py"))
    ...
    # Stage-2a args
    p.add_argument("--stage2a_build_csv_script", type=Path, default=Path("scripts/stage2a/infer/build_stage2a_infer_csv.py"))
    p.add_argument("--stage2a_infer_script", type=Path, default=Path("scripts/stage2a/infer/infer_stage2a.py"))
    p.add_argument(
        "--stage2a_native3_ensemble_infer_script",
        type=Path,
        default=Path("scripts/stage2a/infer/infer_stage2a_native3_ensemble.py"),
    )
    p.add_argument(
        "--stage2a_infer_mode",
        type=str,
        default="native3_ensemble",
        choices=["native3_ensemble", "single"],
        help="Use the selected native 3-class ensemble pipeline by default; single preserves legacy checkpoint inference.",
    )
    p.add_argument(
        "--stage2a_ckpt",
        type=Path,
        default=Path("models/stage2a/stage2a_best_model.pt"),
        help="Single-checkpoint Stage-2a path used only when --stage2a_infer_mode=single",
    )
    p.add_argument(
        ...
        help="Comma-separated native 3-head Stage-2a checkpoints used by the selected ensemble pipeline",
    )
    p.add_argument(
        "--stage2a_native3_population_model_pkl",
        type=Path,
        default=Path("models/stage2a/native3_drop_both_ensemble/population_logfootprint_soft_type_residual_hgb.pkl"),
        help="Packaged log_footprint_m2 + soft native3 type residual-HGB population model",
    )
    p.add_argument(
        "--stage2a_native3_temperatures",
        type=str,
        default="",
        help="Optional comma-separated temperatures for native3 type checkpoints. Empty means no temperature scaling.",
    )
    ...
    p.add_argument(
        "--stage2a_population_blend_json",
        type=Path,
        default=None,
        help="Optional Stage-2a global or source-aware geometry-ridge population blend artifact",
    )
    p.add_argument(
        "--stage2a_footprint_m2_per_mask_px",
        type=float,
        default=4.641392,
        help="Footprint scale passed to build_stage2a_infer_csv.py when deriving footprint_m2 from masks",
    )
    p.add_argument(
        "--stage2a_allow_missing_geometry",
        action="store_true",
        help="Forward --allow_missing_geometry to Stage-2a inference for explicit fallback runs",
    )

    # Stage-2b args
    p.add_argument("--stage2b_infer_script", type=Path, default=Path("scripts/stage2b/infer/infer_stage2_ensemble.py"))
    p.add_argument(
        "--stage2b_ckpts",
        type=str,
        default=(
    ...
    # Presenter args
    p.add_argument("--present_script", type=Path, default=Path("scripts/stage3/present_instance_results.py"))
    p.add_argument("--top_k_uncertain", type=int, default=30)
    p.add_argument("--print_top_n", type=int, default=15)
    p.add_argument("--visualize_script", type=Path, default=Path("scripts/stage3/visualize_stage2_overlays.py"))
...
def subprocess_env():
    ...
    if cwd not in paths:
        env["PYTHONPATH"] = cwd if not current else cwd + os.pathsep + current
    return env
...
def run_cmd(cmd, dry_run=False, verbose=False):
    ...
    env = subprocess_env()
    if verbose:
        subprocess.run(cmd_str, check=True, env=env)
    else:
        subprocess.run(
            cmd_str,
            check=True,
...
def main():
    ...
    if args.stage2a_infer_mode == "single":
        if not args.stage2a_ckpt.exists():
            raise FileNotFoundError(f"Missing Stage-2a checkpoint: {args.stage2a_ckpt}")
        if args.stage2a_population_blend_json is not None and not args.stage2a_population_blend_json.exists():
            raise FileNotFoundError(f"Missing Stage-2a population blend JSON: {args.stage2a_population_blend_json}")
    else:
        native3_ckpts = [Path(x.strip()) for x in args.stage2a_native3_ckpts.split(",") if x.strip()]
        if not native3_ckpts:
            raise ValueError("--stage2a_native3_ckpts produced no checkpoint paths")
        missing = [str(path) for path in native3_ckpts if not path.exists()]
        if missing:
            raise FileNotFoundError("Missing Stage-2a native3 checkpoint(s): " + ", ".join(missing[:5]))
        if not args.stage2a_native3_population_model_pkl.exists():
            raise FileNotFoundError(f"Missing Stage-2a native3 population model: {args.stage2a_native3_population_model_pkl}")
    ...
    run_cmd(
        [
            ...
            str(args.stage2a_footprint_m2_per_mask_px),
        ],
        dry_run=args.dry_run,
        verbose=args.verbose,
    )
    ...
    if args.stage2a_infer_mode == "single":
        ...
        if args.stage2a_population_blend_json is not None:
            cmd_stage2a.extend(["--population_blend_json", args.stage2a_population_blend_json])
    else:
        cmd_stage2a = [
            py,
            args.stage2a_native3_ensemble_infer_script,
            "--input_csv",
            stage2a_input_csv,
            "--ckpts",
            args.stage2a_native3_ckpts,
            "--population_model_pkl",
            args.stage2a_native3_population_model_pkl,
            "--out_csv",
            stage2a_out_csv,
            "--batch_size",
            str(args.stage2a_batch_size),
        ...
        if args.stage2a_native3_temperatures:
            cmd_stage2a.extend(["--temperatures", args.stage2a_native3_temperatures])
    if args.stage2a_allow_missing_geometry:
        cmd_stage2a.append("--allow_missing_geometry")
    run_cmd(cmd_stage2a, dry_run=args.dry_run, verbose=args.verbose)

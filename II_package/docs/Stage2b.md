...
## Model and End-to-End Pipeline Details

This section is a self-contained implementation summary for later manuscript writing. The commands below remain the operational record; this section explains what the scripts do internally and how training-time artifacts become deploy-time predictions.

### Task Definition and Sample Schema

Stage-2b predicts an ordinal building-damage severity class for each building instance:
...
Each training or inference row represents one building instance and carries:

- `pre_crop`: RGB building-centered crop from the pre-disaster image.
- `post_crop`: RGB building-centered crop from the post-disaster image.
- `mask_M`: binary footprint mask for the target building.
- `mask_R`: binary context-ring mask around the target building.
- `damage_class`: integer label `0..3` during training/evaluation; omitted or `-1` for unlabeled inference.
- Metadata keys such as `tile_id`, `event_id`, and `bldg_uid`.

RGB crops are loaded as 3-channel tensors, scaled to `[0, 1]`, and normalized by ImageNet mean/std. Masks are loaded as single-channel binary tensors. The canonical crop size is `256 x 256`. The default ring mask uses `ring_radius_px=48`, built by dilating the building footprint and subtracting the footprint region. The default polygon source is `pre_then_post`, so the building anchor is tied to the pre-disaster footprint when available.

The train/validation split is tile-blocked: whole `tile_id`s are assigned to train or validation. This avoids placing neighboring instances from the same tile on both sides of the split.

### Model Architecture

The Stage-2b model is `SiameseDamageModel` in `scripts/stage2b/training/train_stage2.py`.

Core design:

- Backbone: `convnext_tiny` from `timm`, created with `features_only=True`.
- Weight sharing: the same backbone processes `pre_crop` and `post_crop`.
- Default feature stage: `backbone_stage_index=-1`, i.e. the final backbone feature stage.
- Default pooling: `mask_m_ring`.
- Default fusion: `pre_post_diff`.
- Classifier: CORAL ordinal head with `num_classes=4`.

For each sample, the model computes:

```text
f_pre  = ConvNeXt(pre_crop)
f_post = ConvNeXt(post_crop)
```

For the default `mask_m_ring` setting, masked average pooling is applied separately on the footprint mask `M` and context ring `R`:

```text
pre_M  = avg_pool(f_pre,  mask_M)
post_M = avg_pool(f_post, mask_M)
pre_R  = avg_pool(f_pre,  mask_R)
post_R = avg_pool(f_post, mask_R)
```

The default fused feature vector is:
...
If the selected backbone stage has `C` channels, this default fusion has dimension `8C`. The ablation modes are:

- `rgb_only`: global average pooling over the full feature map, no masks.
- `mask_m`: masked pooling only over the building footprint.
- `mask_m_ring`: masked pooling over both footprint and surrounding ring.

The head is:

```text
Linear(in_dim, 512) -> ReLU -> Dropout(0.1) -> Linear(512, 3)
```

The final layer outputs three cumulative CORAL logits for the four ordinal classes. Logit `k` models the cumulative event `P(y > k)` for `k in {0, 1, 2}`. Class probabilities are reconstructed as:

```text
p0 = 1 - sigmoid(logit_0)
p1 = sigmoid(logit_0) - sigmoid(logit_1)
p2 = sigmoid(logit_1) - sigmoid(logit_2)
p3 = sigmoid(logit_2)
```

The probabilities are clamped and renormalized for numerical stability.
...
```text
raw xBD flood tiles/labels
  -> build_stage2_index.py
  -> preprocess_stage2_crops.py
  -> audit_stage2_samples.py
  -> train_stage2.py
  -> stage2_best.pt, stage2_last.pt, train_config.json, optional validation predictions
```

Training uses the `stage2_samples.csv` generated by preprocessing. The default frozen configuration is the `run019`-style setup:
...
- `class_balance_alpha=0.2`
- `class_balance_cap=3.0` in the sweep record; the packaged run019 config stores its exact saved value.
- `coral_label_smoothing=0.02`
- `ema_decay=0.999`
- `best_metric=macro_f1`
- `best_tiebreak_metric=qwk`

Data augmentation is applied only in training:

- synchronized horizontal flips across pre/post/masks,
- optional synchronized vertical flips,
- synchronized random 90-degree rotations,
- mild brightness/contrast jitter applied independently to pre and post RGB tensors.

The loss is binary cross-entropy with logits over the CORAL cumulative targets:

```text
target = [y > 0, y > 1, y > 2]
```

With label smoothing enabled, binary targets are softened toward `0.5`. Optimization uses AdamW, optional bf16/fp16 AMP on CUDA, cosine learning-rate decay with warmup, and optional distributed training via `torchrun`/DDP.

At the end of each epoch, the model runs on the validation split and reports:
...
- optional per-event metrics and saved validation predictions

`stage2_last.pt` is saved every epoch. `stage2_best.pt` is updated when validation `macro_f1` improves; if `macro_f1` is tied within `best_min_delta`, `qwk` is used as the tie-breaker. When EMA evaluation is active, the best checkpoint stores the EMA weights.

### Calibration Pipeline

Calibration is a post-training probability calibration step. It does not retrain the image model. It reads `val_predictions_best.json` for a checkpoint and fits calibration parameters against the validation labels.

Two calibration families are recorded:

- Temperature scaling: learn one scalar `T`, apply it to CORAL cumulative logits as `logits / T`, then convert back to class probabilities.
- Vector exponents: raise each class probability to a learned class-specific exponent and renormalize.

The calibration script writes:

- `calibration_metrics.json`
- `calibrated_probs_temperature.npy`
- `calibrated_probs_vector.npy`
- `calibration_examples.json`
- `confidence_diagnostics.json`

The packaged inference default uses `temperature` calibration because it improves reliability metrics while preserving the run019 argmax decisions in the recorded validation check. Vector calibration achieved lower NLL/ECE in some runs but can change argmax predictions and reduce macro-F1.

### Inference and Ensemble Pipeline

The packaged deployment path is:

```text
Stage-1 building detections
  -> generate_shared_instance_subimages.py
  -> shared_instance_samples.csv with pre_crop/post_crop/mask_M/mask_R
  -> infer_stage2_ensemble.py
  -> stage2b_ensemble.jsonl
  -> present_instance_results.py and visualization utilities
```

`infer_stage2_ensemble.py` expects exactly three checkpoints and their matching `train_config.json` files. The current default ensemble is:
...
For each model, inference produces:

- cumulative CORAL logits,
- raw class probabilities,
- calibrated class probabilities if calibration artifacts are provided.

The ensemble combines raw cumulative logits by weighted average and maps them back to class probabilities for the predicted damage class:

```text
ensemble_logits = weighted_mean(model_logits)
ensemble_probs  = coral_probs_from_logits(ensemble_logits)
y_pred_ensemble = argmax(ensemble_probs)
```

Confidence and uncertainty summaries are computed from the calibrated probability ensemble:

```text
ensemble_probs_calibrated = weighted_mean(model_probs_calibrated)
expected_severity = sum_c c * ensemble_probs_calibrated[c]
pmax = max_c ensemble_probs_calibrated[c]
margin = top1_probability - top2_probability
entropy = -sum_c p_c log(p_c)
```

Two disagreement metrics are also emitted:

- `var_predicted_class_prob_weighted`: weighted variance across models of the probability assigned to the ensemble-predicted class.
- `var_expected_severity_weighted`: weighted variance across models of expected severity.

The output JSONL stores metadata, `y_true` if available, `y_pred_ensemble`, expected severity, confidence metrics, uncertainty metrics, per-model logits/probabilities, ensemble logits/probabilities, and the calibration method.
...
Script:
- `scripts/stage2b/infer/infer_stage2_ensemble.py`
...
```bash
python3 scripts/stage2b/infer/infer_stage2_ensemble.py \
  --csv outputs/flood_stage2_prep_r48/stage2_samples.csv \
...
Run:
- `scripts/stage2b/infer/infer_stage2_ensemble.py` with:

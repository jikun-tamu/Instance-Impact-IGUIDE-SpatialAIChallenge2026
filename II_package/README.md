...
- `scripts/driver/` - end-to-end package driver
- `scripts/shared/` - shared instance crop/mask generation
- `scripts/stage2a/training/` - Stage 2a experiment/training utilities
- `scripts/stage2a/infer/` - Stage 2a deployable inference utilities
- `scripts/stage2b/training/` - Stage 2b training entrypoint
- `scripts/stage2b/infer/` - Stage 2b ensemble inference entrypoint
- `scripts/stage3/` - presentation, visualization, and aggregation utilities
- `stage1/SAM3_Final_20260226/` - Stage 1 code used by the driver
- `models/stage2a/` - Stage 2a native3 ensemble checkpoints and population artifact
...
The launcher calls `scripts/driver/run_instance_impact_driver.py` with package-local defaults:

- Stage 2a mode: native3 drop-both five-checkpoint ensemble
- Stage 2a ckpts: `models/stage2a/native3_drop_both_ensemble/seed*/stage2a_best_model.pt`
- Stage 2a population model: `models/stage2a/native3_drop_both_ensemble/population_logfootprint_soft_type_residual_hgb.pkl`
...
- `--stage2a_infer_mode`: keep `native3_ensemble` for the packaged baseline.
- `--stage2a_native3_ckpts`: keep the packaged five-checkpoint native3 ensemble unless intentionally swapping members.
- `--stage2a_native3_population_model_pkl`: keep the packaged residual-HGB population artifact.
- `--stage2a_ckpt`: legacy single-checkpoint path used only with `--stage2a_infer_mode single`.
